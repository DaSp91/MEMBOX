...
int icl_hash_dump(FILE* stream, icl_hash_t* ht){
  ...
  for(i=0; i<ht->nbuckets; i++) {
    ...
    for(curr=bucket; curr!=NULL; ) {
      if(curr->key){
        message_data_t * rawData = (message_data_t *) curr->data;
	
	if(fwrite((unsigned long*)(curr->key),sizeof(unsigned long),1,stream) == 0)
		return -1;
	
	
	if(fwrite(&(rawData->len),sizeof(unsigned int),1,stream) == 0)
		return -1;
	
	
	if(fwrite((rawData->buf), sizeof(char),rawData->len,stream) == 0)
		return -1;
      }
      curr=curr->next;
    }
